...
#### 3.2bis 两张表对同一个口径的处置相反，而**不设防的那张是权威表**（2026-08-31 查明）
...
所以这是一条**触发条件已知的定时缺陷**：P3 一旦登记可路由模型，这条路径立刻开始
写入，而它写的口径可能是被默认值伪造出来的。⚠️ **P3 动成本计算前必须先修表**
（`price_versions.billing_unit` 改可空 + 补 CHECK + 去掉默认值，并把 `sink.go` 的
`COALESCE` 换回 `NULLIF`）。**不要等成本算错了再回头查** —— 两种口径的数值区间
实测重叠（按次 $0.004~7 / 倍率 0.01~175），错了看不出来。

未在本轮修：改的是 P3 的消费路径与一张 P1 用不到的表，且要新迁移动 `NOT NULL`
约束；发版前动它属于范围外，收益也只在 P3 才兑现。**已登记在此，不靠记忆。**
