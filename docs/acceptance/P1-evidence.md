...
### 3.4 真库里有一行夹具残留，31 项里的 3 条 Key 断言跑在它身上（2026-08-30 查明）
...
2. **31 项里的 3 条 FR-094 断言（Key 列表有行 / 不回显明文 / 前缀截断）跑在夹具
...
移出真库套件、只留在 `verify-ui.mjs`。选 ② 要接受真库套件从 31 项降到 28 项。
...
第二处顺带查出真库里有一行夹具残留，见 [§3.4](#34-真库里有一行夹具残留31-项里的-3-条-key-断言跑在它身上2026-08-30-查明)。
