...
| 证据留档 | ✅ 本文件 + [P1-manual-channels.md](P1-manual-channels.md)（人工清单）+ 三轮覆盖率报告归档在 [`coverage/`](coverage/)。截图仍只在 `/tmp/sla-ui-shots/`（8 张 + `results.json`，含 fullPage）—— **未入库**，因为含真实站名与额度，且每轮覆写 |
...
| 全渠道 sync 覆盖率报告 | ✅ 见 §2.1（三轮已归档进仓库） |
| 采不到的渠道列出人工维护责任人（#12 第 4 项） | ⚠️ **清单已出，责任人未指派** —— 20 个站逐行列在 [P1-manual-channels.md](P1-manual-channels.md)，含类别与下一步动作；**责任人一列空着**：谁维护哪个站是账号归属与商务关系，库里与导出里都没有承载它的字段。按 14 §3 规则 3「不可判定即不通过」，编一个名字比留空更坏。这一项的结清需要运营侧补人名 |
...
移出真库套件、只留在 `verify-ui.mjs`。选 ② 要接受真库套件从 32 项降到 29 项。
...
| `verify-remote.mjs`（内网真库只读） | **32/32** | SLA_DB @ <internal-db-host>（PG 17.5）；靶子每轮由 `pickTargets()` 现查，见 §3.3 |
| `ui-stack.sh` 末步：Key 明文不进 core 日志 | **✅** | 退出标准③ 的"日志"那一端，2026-08-29 补 |
| 合计 | **104 项** | 2026-08-30 从 103 增至 104：`verify-remote.mjs` 补了一条「筛选关键词不在首页行内」的**前提断言** —— 关键词改为推导后，若它恰好落在首页，「筛选走后端 q 参数」就退化成「前端在当前页里过滤也能绿」。破坏性验证里这条正是这样红的，而它守的那条断言当时仍然绿。 |
...
| `remote-stack.sh`（内网真库只读） | ✅ **32/32** | 见下方生命周期那段 |
