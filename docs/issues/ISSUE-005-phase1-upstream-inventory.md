...
| 使用方式 | **只在 §0 速览表"决定"列填** ✅ 或替代答案。§1 的术语分层必须先答——它决定后面所有文档怎么标 |
...
| # | 事项 | 推荐方案 | 决定 |
| --- | --- | --- | --- |
| P0 | **术语分层**（必答） | 原「一期」31 条 AC **升为最终目标**（需求范围，不代表交付顺序）；交付进度另立 **P1~P4**。现有文档 249 处「一期」/82 处「二期」**一字不动**，只在 PRD §2.1 加一张映射表。理由见 §1 | |
| P1 | P1 交付边界 | **完全不含请求转发**：不碰 `/v1/*`、不碰账本、不碰调度、不碰 SLA。只做"把上游资产采准、可管、可查" | |
| D1 | 充值倍率 | **P1 不做**（照指示①）。代价签收：1:2 充值渠道成本会被高估 2 倍——但 P1 无成本排序，**无消费者**，故不是缺口。登记为 P2 任务（§6） | |
| D2 | 分组实体 | 新增 `channel_groups`（**6 列**，砍掉高峰倍率等 6 字段）+ `group_models`（3 列） | |
| D3 | Key 用量 | `upstream_keys` 补 6 列存**当前值**；历史时序**复用已有 `collector_snapshots`**（`scope_type='key'`），不建新表 | |
| D4 | 渠道模型目录 | 新增 `channel_model_catalog`（**6 列**，砍掉能力位与启用关联） | |
| D5 | Key 级限流 | P1 **只采集存储**（`upstream_keys.rpm_limit`/`concurrency_limit` 两列），不做派生、不做判闸——判闸属容量保留，是网关的事 | |
| D6 | 「实时更新」语义 | 上游无 webhook → 不做推送式实时。`POST /admin/channels/{id}/sync` 手动立即刷新 + 周期采集（FR-116），界面显示 `synced_at` 与陈旧标记 | |
| D7 | 密钥管理边界 | P1 做**上游 Key** 的 CRUD＋轮换＋停用＋脱敏展示。入站凭证 `gateway_clients` 属网关，**P1 不做** | |
| N1 | 新增 FR | FR-122~FR-128（7 条，§4） | |
| N2 | 新增 AC | AC-37~AC-40（4 条，§4） | |
