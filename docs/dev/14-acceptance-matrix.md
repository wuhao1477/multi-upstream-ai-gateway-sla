...
| AC-18 | **1000 并发用户高强度持续使用**（一期无主动测活） | **LOAD** | 按 §2bis 冻结模型：① 1000 并发稳态 30 分钟，决策开销 **P99 ≤50ms**、错误率 <0.01%；② 内存不持续增长、结束后 goroutine/连接回落基线 ±10%、无 OOM/FD 耗尽；③ 容量保留生效；④ **另须完成上限探测并记录容量拐点** |
