...
| 作用 | 把 [PRD](../PRD.md) 的 32 条 AC **逐条**绑定到里程碑、验证环境、判定阈值。**开发说"做完了"时，PM 据本表判定真假** |
...
| **M2 流式 SLA 核心** | AC-06、AC-07、AC-09、AC-12、AC-15、AC-25 | 6 |
| **M3 元数据与订阅** | AC-02、AC-03、AC-04、AC-05、AC-17、AC-19、AC-20、AC-21、AC-22、AC-23、AC-24、AC-28、AC-29 | 13 |
| **M4 经营与验收** | AC-08、AC-10、AC-11、AC-13、AC-14、AC-18 | 6 |
| 合计 | | **32** |
...
| AC-25 | `gpt-5.5`（可测活）vs `gpt-5.5-sla-1`（禁测活） | FIXTURE | 各发 100 次：前者出现 `probe_kind='probe'` 的 attempt，后者**0 次**；两者适用各自别名映射的 SLA 等级 |
...
| AC | 场景 | 环境 | 判定方法（可执行） |
| --- | --- | --- | --- |
| AC-08 | 低权重渠道长期无样本 | FIXTURE | 样本不足（1h<20 且 24h<100）→ `low_confidence=true`，**不作主渠道**；仅在测活预算内获得试探流量 |
