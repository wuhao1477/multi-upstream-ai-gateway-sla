...
| 项目 | 内容 |
| --- | --- |
| 状态 | ✅ **v1.0 基线（2026-07-26 冻结）** —— 经 28 轮对抗性审查 + 2 轮开发视角走查 + PM 开工前裁决；变更须走版本记录；一期 31 条 / 二期 5 条已冻结 |
| 日期 | 2026-07-25 |
...
> **2026-07-26 范围调整**：主动测活（AC-09/10）与缓存切换损失预测（AC-07）**由二期拉入一期**；订阅制（AC-20~24）维持二期——现有订阅渠道基本已到期且暂不续费，一期无订阅渠道可调度。一期 28 → **31**，二期 8 → **5**，合计仍 36。
...
| AC | 场景 | 环境 | 判定方法（可执行） |
| --- | --- | --- | --- |
| AC-06 | A 渠道 ~20s 低价、B 渠道 ~3s 较贵 | MOCK | 金级别名请求：首跳选 A，期限内未见有效首字 → 切 B；**最终 TTFT < 该等级目标**；账本含 2 条 attempt（#1 `canceled_by_sla`、#2 `committed`）。<br>**接管后的费用与占用**（[02 §2bis](./02-data-model.md) `closeout_attempt`）：①`actual_usd` 须**包含 hop1 的实际成本**（hop2 的 finalize 会释放整笔 RoutePlan 预留，只记 hop2 usage 会让 hop1 的钱凭空消失、settled_usd 系统性偏低）；②hop1 若是 canary，其 claim 须在**该跳结束时立即** `released`、`canary_inflight` 同步扣减，**不得依赖 60s 租约过期回收**；③hop1 的 `attempt_status='canceled_by_sla'` 且有对应 `attempt_usage` 行 |
...
| AC-25 | `gpt-5.5` vs `gpt-5.5-sla-1` 两个别名 | FIXTURE | 各发 100 次：两者**适用各自别名映射的策略**（一期为单级 SLA + 测活资格标记）；**测活资格差异属一期验证**：`gpt-5.5` 可被分配测活请求、`gpt-5.5-sla-1` 一次都不得被分配 |
