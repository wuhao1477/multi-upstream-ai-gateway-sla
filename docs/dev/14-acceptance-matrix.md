...
| **M4 经营与验收** | AC-08、AC-11、AC-13、AC-14、AC-18、**AC-34**（1000 并发抗压） | 6 |
| 合计（一期） | | **28** |
...
| **AC-33**（新增） | 入站鉴权：无凭证 / 已吊销 / 越权别名 / 超配额 | FIXTURE | ① 不带 `Authorization` → **401**；② 用已 `revoked` 凭证 → **401**；③ 用 `allowed_aliases` 外的别名 → **403**；④ 超 `rpm_limit` → **429**；⑤ 合法凭证正常 200。另断言：`GET /admin/clients` **不回显完整凭证**、库中 `secret_hash` 非明文、日志中不出现凭证明文 |
...
| **AC-35** | 进程在三个时点崩溃后重启（**账本崩溃恢复**） | FIXTURE | 自动化在 ①上游调用前 ②首字后终帧前 ③终帧后关单前 `kill -9` core，重启后分别核验：①可查到 `pending` 请求意图（判定为疑似已计费）②有 TTFT 无 usage ③完整记录。**三种情况都不得出现"请求完全不存在"**；`ledger_outbox` 未投递行被重放且**不产生重复账目**（[01 §5.1](./01-architecture.md)、[02 §9.2bis](./02-data-model.md)） |
