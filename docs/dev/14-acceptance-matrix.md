...
> **文档性质：全阶段验收定义，不是当前通过记录。** P2/P3/P4 条目只有在对应实现完成并重新执行后才能标记通过；当前证据见 `docs/acceptance/`，已交付范围见 [README 路线图](../../README.md#路线图)。

| 项目 | 内容 |
| --- | --- |
| 状态 | ✅ **v1.0 基线（2026-07-26 冻结）** —— 经 42 轮对抗性审查（含 5 轮开发视角）+ PM 开工前裁决；变更须走版本记录；**一期 35 条（含 P1 的 AC-37~40）/ 二期 5 条**（2026-07-27 ISSUE-005 交付切分后） |
| 日期 | 2026-07-25 |
| 缘起 | PM 视角审查发现：AC 未与里程碑挂钩，[00 M4](./00-overview-and-milestones.md) 写着"验收矩阵全绿"但**该矩阵不存在** → 每个里程碑无法闭环验收 |
| 作用 | 把 [PRD](../PRD.md) 的 **AC-01～40 全集**逐条绑定到里程碑、验证环境、判定阈值。**每条 AC 恰好归属一个需求分期与一个交付阶段**（见 §1 总览）。开发说"做完了"时，PM 据本表判定真假 |
...
> **两个维度并存**（[PRD §2.1.0](../PRD.md)）：「一期/二期」是**需求范围**，**P1~P4** 是**交付进度**。下表的里程碑名（M0~M4）与期别标签**语义不变**，只加一列阶段归属。

| 里程碑 | 交付阶段 | 验收 AC | 条数 |
| --- | --- | --- | --- |
| **M0 骨架** | **M0** | AC-27、**AC-33-M0**（入站鉴权基础子集，见下方拆分说明） | 2 |
| **P1 上游采集与管理** | **P1** | **AC-37**、**AC-38**、**AC-39**、**AC-40**（v1.5 新增，见 §2 P1 段） | 4 |
| **M1 上游直连 + 账本** | P2 | AC-01、AC-16、AC-26、AC-30、AC-31、AC-32、**AC-35**（崩溃恢复）、**AC-33-M1**（配额与结算子集） | 7（AC-33 与 M0 共享编号，不重复计数） |
| **M2 流式 SLA 核心** | P2 | AC-06、AC-12、AC-15、AC-25、**AC-07**（缓存切换损失预测） | 5 |
| **M3 元数据采集** | P3 | AC-02、AC-03、AC-04、AC-05、AC-17、AC-19、AC-28、AC-29 | 8 |
| **M4 经营与验收** | P3 | AC-08、AC-11、AC-13、AC-14、AC-18、**AC-09**、**AC-10**（主动测活）、**AC-34**（1000 并发抗压）、**AC-36**（1000 QPS 吞吐） | 9 |
| 合计（一期） | | | **35** |
| ⏭ **二期（订阅制）** | P4 | AC-20、AC-21、AC-22、AC-23、AC-24 | 5 |

> **2026-07-26 范围调整**：主动测活（AC-09/10）与缓存切换损失预测（AC-07）**由二期拉入一期**；订阅制（AC-20~24）维持二期——现有订阅渠道基本已到期且暂不续费，一期无订阅渠道可调度。一期 28 → 31，二期 8 → **5**。
>
> **2026-07-27 交付切分（[ISSUE-005](../issues/ISSUE-005-phase1-upstream-inventory.md)）**：新增 **P1 阶段 4 条 AC-37~40**（上游采集与管理）。它们属**最终目标内必做项**，故计入"一期"侧：一期 31 → **35**，二期仍 5，**合计 36 → 40**。原 M1~M4 的归属、判定与退出标准**一字未改**，只增加阶段列。

---

## 2. 逐条验收定义

### P1 上游采集与管理（v1.5 新增，[ISSUE-005](../issues/ISSUE-005-phase1-upstream-inventory.md)）

> **P1 边界**：不含任何请求转发——不碰 `/v1/*`、不碰账本、不碰调度、不碰 SLA。故本段 4 条 AC **全部只验管理平面与采集结果**。
...
| AC | 场景 | 环境 | 判定方法（可执行） |
| --- | --- | --- | --- |
| AC-27 | 自研核心某一实例宕机 | FIXTURE | `docker stop sla-core-a` → 持续打 30 个请求，**全部成功、无一失败**；Caddy 日志显示已摘除该实例；恢复后自动重新纳入。<br>**打哪个端点**（第 45 轮补——此前未写，而 Caddy 只代理 `/v1/*` 与 `/healthz`，`/v1/*` 到 P2 才存在，开发无从确定）：**M0/P1 阶段打 `GET /healthz`**（当时唯一经 LB 的端点）；**P2 起改打 `/v1/chat/completions`**（上游用 `sse_stream_fixture.py` 起流，本条测的是 LB 摘除而非上游）并**在 P2 重跑本条**——`/healthz` 只能证明 LB 摘除生效，证明不了请求路径在实例宕机时不丢，而后者才是 FR-110「任一 core 宕机不影响服务」的实质。两次都须留存证据。<br><br>⚠️ **2026-09-11 覆盖范围变更（不是撤销）**：默认部署形态（根目录 `compose.yml`）已改为**单实例 + 外部库**，不再起 Caddy 与第二个 `sla-core`，因此**默认部署不再满足本条** —— 重启即中断。变更理由：外网已有 NGINX 一层，Caddy 的 TLS 与之重复，而 P1 经 LB 对外只有 `/healthz`。**本条并未作废**：冗余拓扑仍完整保留在 `deploy/docker-compose.yml`，`verify/test-compose.sh` 的 4/5 段继续真跑并留证据（已通过记录见 [P1-evidence](../acceptance/P1-evidence.md)）。P2 若要重新承诺 FR-110，需把第二实例**与**一层带主动健康探测的 LB 一并加回默认形态 —— 缺任一条本条都不成立。 |
...
| AC | 场景 | 环境 | 判定方法（可执行） |
| --- | --- | --- | --- |
| AC-08 | 低权重渠道长期无样本 → 受控验证闭环（**FR-121**） | FIXTURE | ①样本不足（1h<20 且 24h<100）→ `low_confidence=true`，**不作稳定主渠道**；②**新建 binding** 与**冷却期满 binding** 都进入 `canary`，在**不制造任何全局故障**的前提下，仅靠正常业务流量走完 `canary → observing → available`（[05 §2.0](./05-scheduling-and-operations.md)）；③canary 硬上限生效：单 binding 每小时 ≤`canary_max_per_hour`、并发 ≤`canary_max_concurrent`（**口径为 per-binding，非 per-channel**，A5 裁决），**`is_committed=true` 的承诺档别名请求一次都不得**落到 canary binding（**读策略字段不读等级名**；M0 种子里即 `gpt-5.5-sla-1`）；④canary 连续失败达阈值 → 退回 `cooling` 且退避翻倍；⑤无健康接管候选时**不分配** canary；⑥**双 core 高竞态**：两实例同时对同一 canary binding 打 200 请求，实际落到该 binding 的并发**恒 ≤`canary_max_concurrent`**、每小时**恒 ≤`canary_max_per_hour`**（原子 claim 生效，[05 §2.0](./05-scheduling-and-operations.md)）；⑦**claim 与 attempt 同事务**：在 claim 成功、attempt 未落库之间 kill，重启后 `canary_inflight` 不泄漏且 `canary_claims` 无孤儿行；⑧请求结束/租约过期回收后 `canary_inflight` 归零，且回收**不得**误伤尚在事务中的 claimant；⑩**dispatch 事务原子性**：canary 请求的 预留／`canary_claims` 行／`canary_inflight` 递增／`attempts` 行／`stage='dispatched'` **五者同事务成败**——在链中任一点 kill，重启后五者要么全有要么全无，不得出现「有预留无 claim」「有 claim 无 attempt」「attempt 已建但 stage 仍 planned」；⑨**长流不被误回收**：一个持续 >5 分钟的 canary 流式请求期间，claim 随 attempt 心跳持续续租、不被后台回收，并发上限全程不被突破 |
...
> **2026-07-26 调整**：FR-056（切换损失预测 + 缓存率目标约束）**拉入一期**，故本条属一期验收。触发原因：主力场景为 Codex 长会话，prompt 前缀长、缓存命中对成本影响显著，只做会话粘性不够。

| AC | 场景 | 环境 | 判定方法（可执行） |
| --- | --- | --- | --- |
| AC-07 | 切低价渠道会使缓存率低于目标（**一期**，FR-056） | FIXTURE | ①构造缓存作用域不可延续的低价候选：`selector` **不选**它，决策快照 `excluded` 含缓存原因；②**须真的做预测而非只看布尔**——断言快照里记录了「预测切换后缓存命中率」与「目标值」两个数，低于目标才抑制；③更高等级 SLA 要求接管时**允许**切换，但须记录切换原因与预计缓存损失；④**长会话场景**（≥5 轮、共享长前缀）下连续请求的实际缓存命中率不因调度切换而低于目标 |

### 主动测活相关（**一期第二轨**，[15 S1](./15-scope-and-preflight.md)）
...
**测活并发（AC-18 原文提及）属一期**（2026-07-26 调整）：压测须同时开启主动测活，断言测活流量在预算与并发上限内、且不影响主流量 P99 与错误率（[05 §2.1](./05-scheduling-and-operations.md)）。
