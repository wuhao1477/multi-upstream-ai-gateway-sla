...
> **文档性质：全阶段目标设计。** 本文包含尚未实现的 P2/P3/P4 能力，不代表当前运行功能。当前已交付范围以 [README 路线图](../../README.md#路线图) 与 `migrations/` 为准。
...
| 技术栈决策（2026-07-23 确认） | 自研核心 **Go**（存储层 **pgx**，手写 SQL；2026-08-29 翻掉原定的 sqlc，见 [10 §5 开放点 3](./10-project-structure.md#5-开放点)）；状态存储 **PostgreSQL 单库**（一期不引 Redis）；部署 **单机 Docker Compose**（LB + **≥2 核心实例** + PG + collector，**无外部网关**）；文档按 docs/dev/ 分篇 |
...
| [16 版本路线图](./16-version-roadmap.md) | 把本篇的交付阶段细切成 v1.1~v6.0：每版一条主线风险 + 出口判据；网关/决策层/SLA 三个大设计的开工前确认清单（7 处待拍板缺口）。**不改需求、不改验收判定，只改执行顺序** | 🟡 草案 |
