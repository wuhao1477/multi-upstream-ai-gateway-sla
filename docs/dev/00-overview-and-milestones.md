...
| **M4 经营与验收** | 冷却/样本门槛、**新渠道受控验证 canary（FR-121，跨实例原子 claim）**、容量保留、告警 P1～P3（落库）、一期验收矩阵收口、100～1000 QPS 压测 | 参数 5/11/12/16、FR-114、**FR-121** | ① [14 验收矩阵](./14-acceptance-matrix.md) 的**一期 28 条 AC 全绿**且证据留档（二期 8 条：订阅 AC-20~24、主动测活 AC-09/10、缓存进阶 AC-07 不在本期；28+8=36 与 [PRD](../PRD.md) 对齐）；② **AC-08 canary 闭环通过**（新建/冷却期满渠道在无全局故障下完成晋级，硬上限双实例不被突破）；③ LOAD：**1000 并发用户高强度稳态 30 分钟**，决策开销 P99 ≤50ms、错误率 <0.01%、无内存/goroutine/FD 泄漏；④ 完成容量上限探测并记录拐点（[14 §2bis](./14-acceptance-matrix.md)） |
