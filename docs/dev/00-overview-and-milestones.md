...
| 8 | 并发重试默认全局禁止；策略全部可配置 + 关键项二次确认（⏭ 测活预算随主动测活移二期） | 参数 6、FR-115 |
| 9 | 模型别名即策略载体（别名映射策略；一期为单级 SLA + 测活资格标记） | FR-062/117、AC-25 |
| 10 | ⏭ **二期**：订阅双倍率（用满倍率调度、实际倍率账务）—— 一期成本排序仅用普通渠道价格版本×倍率 | 参数 14、AC-24（[PRD §2.1](../PRD.md) 二期） |
...
| **M2 流式 SLA 核心** | 内容感知 TTFT、动态期限、首字前接管、mid-stream 取消传播、取消口径归并 | 硬约束 4/5/7，AC-30～32 | ① MOCK 场景全绿（role-only/心跳/空 SSE/慢首字/中断/abort）；② **AC-06/12/15/25 全通过**（[14](./14-acceptance-matrix.md) 一期集；AC-09/10 随主动测活、AC-07 随缓存进阶 FR-056 移二期）；③ 内容感知 TTFT 在三类元事件场景下**均不出现 ≈0ms** |
...
| **M4 经营与验收** | 冷却/样本门槛、容量保留、告警 P1～P3（落库）、一期验收矩阵收口、100～1000 QPS 压测 | 参数 5/11/12/16、FR-114 | ① [14 验收矩阵](./14-acceptance-matrix.md) 的**一期 25 条 AC 全绿**且证据留档（二期 7 条：订阅 AC-20~24、测活 AC-09/10 不在本期）；② LOAD：**1000 并发用户高强度稳态 30 分钟**，决策开销 P99 ≤50ms、错误率 <0.01%、无内存/goroutine/FD 泄漏；③ 完成容量上限探测并记录拐点（[14 §2bis](./14-acceptance-matrix.md)） |
...
| [14 验收矩阵](./14-acceptance-matrix.md) | **AC → 里程碑 → 可执行判定方法**；PM 与开发的验收契约（一期 25 条 + 二期 7 条） | 草案（M1 前定稿） |
