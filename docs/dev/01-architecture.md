...
| 2 | AxonHub 对 **OpenAI Responses 协议**的透传完整度 | ✅ **已用真实上游实测收口**（[07 §3bis](./07-axonhub-runtime-probes.md)，2026-07-25）：inbound `/v1/responses` 原生支持；outbound 须配 `openai_responses` 渠道（`openai` 型静默下转 CC）。**但 round-trip 损耗严重：顶层字段 35→7（丢 `prompt_cache_key`/`previous_response_id`/`reasoning` 等 28 项），`output` 的 reasoning item 被整条吞掉、`reasoning_tokens` 归零**。→ 需 Responses 专有语义时**必须 protocol 层直连**（非可选）；主力 Codex 的会话标识走 HTTP 头不受影响 |
