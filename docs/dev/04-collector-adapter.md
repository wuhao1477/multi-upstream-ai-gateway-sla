...
> **一条总原则（承接 ISSUE-002 前置结论）**：上游站点异构程度高，**必须按站型分流**。NewAPI/Sub2API 是通用开源项目，同族站点复用同一适配器但接入前必须先探测确认；ASXS 是闭源自建平台，**一站一适配器，不可复用、不作探测基准**。任何不支持的字段返回 `unsupported`，**不静默留空**（与 GatewayAdapter 的 `degraded_fields` 原则一致）。
