...
| `GET /admin/catalog?q=&unit=&channel_id=&vendor=&endpoint=&key_id=&sort=&limit=&offset=` | **跨渠道**的模型目录：一行一个模型，带 `channel_count` / `stale_count` / `disabled_count` 与有它的全部渠道明细（渠道名、价格、口径、是否疑似下架、渠道状态，以及**该渠道下能调到这个模型的分组与各自的分组倍率** `groups[]`）。⚠️ 明细里的 `input_price` 是**模型自己的倍率**，实际计费是 `模型倍率 × 分组倍率`，后者由 Key 所在分组决定 —— 实测 2026-09-14 两个真站点的分组倍率跨度是 0.12~1.5 与 0.26~3.5，同一个模型换个分组差十几倍。分组归属取自 `group_models`（NewAPI 由每个模型的 `enable_groups` 反转得来），不是"把渠道下所有分组都挂上"：后者会显示一个调不到这个模型的分组连同它那个漂亮的倍率，而那是选不到的价格。`groups` 为空 = 还没采到分组，**不可当成 ×1**。明细还带该渠道的 `quota_per_unit`（Detect 从 `/api/status` 采的额度换算基数），界面据此把倍率折成**绝对美元价**：`每 1M token 美元价 = 模型倍率 × 分组倍率 × 1e6 / quota_per_unit`。⚠️ 倍率**跨站点不可比** —— 同一个「×1」，基数 500000 的站上是 $2/1M，250000 的站上是 $4，而两边界面都写着 ×1；这正是「有些中转站把倍率设成 ×1 但实际是官方 0.5 或 2 倍」的来源。没采到基数的渠道**不折算**（字段缺席），界面只显示倍率 —— 拿 500000 兜底会在改过基数的站上给出一个看起来精确的错价。按次计价不过这条公式（上游那边 `model_price × quota_per_unit × group_ratio`，除回去就是 `model_price × group_ratio`，与基数无关）。 `channel_id` / `vendor` / `endpoint` 是逗号分隔的多选筛选，响应回四组分面计数，每组都**在除自己以外的全部筛选之下**统计（算进自己那一维的话，选中一个供应商后其余全变 0，就换不了供应商）。`key_id=3,7` 是**按 Key 筛**：只留这几把 Key 所在分组调得到的模型（`group_models`），多把取并集，且明细里的 `groups[]` 只保留这些 Key 自己的分组 —— 那时人问的不是"这个模型最便宜能到多少"，而是"我这把 Key 调它要多少钱"。Key 自己没有分组时**回落到账号的默认分组**（`upstream_accounts.account_group`，来自 `/api/user/self` 的 `group`）—— 上游 `/api/token` 的 `group` 为空串时调用走的就是账号那一档，所以那种 Key 不是「未归组」，它有确定的倍率与可调模型，只是**改法不同**（账号分组变了它跟着变），界面标「跟账号」。⚠️ 两级都解析不出来时（账号分组也没采到，或它不在该站 `group_ratio` 里）才**什么都匹配不到**，这是对的：不知道分组就不能替它断言能调什么。实测 2026-09-14 两个真站点导入的 6 把 Key 里有 3 把如此（上游 `/api/token` 的 `group` 是空串、站点用 `auto_groups` 在调用时自动选组；或它声明的分组不在我方采到的 `group_ratio` 里）。界面据此把这种 Key 的分面 chip 与 Key 管理页的入口按钮**禁用并说明原因**，而不是让它筛出一个无法解释的空列表。与上一行是同一张 `channel_model_catalog` 的两个方向 —— 那个问"这个渠道有什么模型"（建站视角），这个问"这个模型哪些渠道有"（选型视角）。⚠️ **端点名不是 `/admin/models`**：那是「可路由模型」那张表的名字，目录与它是两层（[02 §1.3](./02-data-model.md)）。没有 `stale=true` 这一位 —— 全局视角下陈旧是**逐渠道**的属性（同一个模型可能在 A 站在架、B 站疑似下架），筛成布尔值会丢掉这个区别，所以只给计数。`units` 同上一行在分段筛选**之前**统计；`whole` 是不分段时的模型数，**不等于 `units` 各项相加**（一个模型两种口径都有时会被数两次）。`sort=` 是排序：缺省/`channels` 按支持它的渠道数降序（先回答"哪些模型到处都有"）、`name` 按模型名、`price` 按价格。⚠️ **`price` 只在同时给了 `unit` 时才生效**，否则**静默降级回缺省** —— 两种口径的数值区间重叠（实测按次 0.004~7 vs 倍率 0.01~175），跨口径按价格排会把 $7/次 的视频模型排在"倍率 175"之前。每种排序都以模型名收尾，否则同值行在两次查询间顺序可变，翻页会漏行也会重复行。响应回显 `sort`（降级后的值），界面据此把"按价格"那一项在未选口径时置灰。分面响应里另有 `vendor_icons`（发行方 → 上游 `vendors[].icon` 的图标名），同一家在不同站点标了不同图标名时取出现最多的那个（跳过没标的）；界面按它取仓库内置的 SVG，取不到就退回名字取色的字母块 —— 不发外链（离线保证）。聚合与分页都在 SQL 里做：全局是几万行，拉进进程只为切 50 行不划算 | **P1** |
...
| `GET /admin/version` | **正在跑的这个二进制的版本**（`main.version`，由 `-ldflags` 注入；没注入时回 `dev`）。管理界面侧栏显示它。为什么向后端要而不是编进前端：界面由 `go:embed` 打进二进制，但运维要回答的是"这台在跑哪一版"，而前端构建期常量只能回答"这份界面是哪一版编的" —— 滚动升级或手工换二进制时两者会不一样，而那正是需要查版本的时候。与其它 `/admin/*` 一样要令牌（§1 的口径不为它开口子） | **P1** |
