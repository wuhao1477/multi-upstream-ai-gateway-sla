...
> **文档性质：后续路由与经营目标设计。** 本文描述的 selector、SLA 接管、测活和告警尚未在 P1 交付；不要与 P1 的渠道采集调度混淆。当前已交付范围以 [README 路线图](../../README.md#路线图) 为准。
...
- **`routing_policies` 一期用「原地更新 + 版本号自增」，不做插入式版本化**（第 41 轮修正我自己上一轮引入的矛盾）：
  ...
  | 项 | 一期做法 |
  | --- | --- |
  | 改策略 | **`UPDATE` 该行**，同时 `version = version + 1`、写 `changed_by`/`change_reason`/`effective_at`；**同事务向 `routing_policy_revisions` 追加一条改前快照** |
  | `is_active` | **纯启停开关**，不承载版本语义。置 false = 该策略下线，指向它的别名返回 **503** |
  | 生效行 | 就是 `model_aliases.policy_id` 指的那一行，**不需要再按 name 找版本** |
  | 变更历史 | **影子表 `routing_policy_revisions` 只追加**（[02 §2](./02-data-model.md)），承载 FR-104 的"版本/生效时间/变更原因/责任人" |
  | 回滚 | `POST /admin/policies/{id}/rollback?to_version=N` —— 把该 revision 行的字段抄回主行、`version+1`、同时把**当前值**也存一条 revision。**回滚本身也是一次变更** |
  | 历史请求的可回溯 | 靠 `requests.sla_level`（决策时的冗余快照）+ `requests.decision_snapshot`，**不靠**回查策略表 |

  > ⚠️ **为什么不能"一期不留历史"**（第 42 轮修正我上一轮的裁定）：上一版为了保外键完整性，
  > 写了"一期不保留变更史"。但 [FR-104](../PRD.md) 是 **P0** 且明写"**支持恢复到上一个已确认版本**"——
  > 原地覆盖之后旧值就没了，改错 `probe_allowed` 或 `conflict_order` 无法回退，FR-104 直接无法验收。
  > 影子表同时满足两件事：**主表单行身份不变**（两个外键安全）+ **历史只追加**（可回滚）。
  > 这是插入式版本化与原地更新之外的第三条路，两难本来就不该二选一。
...
| 7 | 容量保留 | 扣除接管保留与**承诺保留**（`is_committed` 策略专用）后仍有余量；**未登记容量的渠道不做闸门判定、本层直接放行**（§4.3）。⚠️ "不做判定"**不等于**"不写 claim"——claim 仍照写（它是租约与悬挂回收的载体），只是不递增/不判定 `resource_health` 计数，见 [02 dispatch 变体 B](./02-data-model.md) | 参数12 |
...
- canary attempt 落 `attempts.role='canary'`，**正常计费、正常计入成本**；若该跳被 SLA 接管取消，其 claim 由 [`closeout_attempt`](./02-data-model.md) **在该跳结束时立即释放**，不等租约过期；其失败**计入该 binding 健康统计**，但因别名限定为 `canary_eligible`（必然 `is_committed=false`，见 §1.1 与 [02 §2 CHECK 约束](./02-data-model.md)），**不消耗任何承诺档的错误预算**。
...
| 会话 | 每会话最多被测活 **1** 次；会话第一轮、**承诺档（`is_committed=true`）会话**默认不测活 | `session_prefix_ledger` / 会话级标记 |
...
- **前置条件（FR-061，全满足才可测活）**：模型能力匹配 + 数据许可 + 价格有效（新鲜）+ 余额充足 + 容量充足（不占接管保留）+ 会话可承受（**非承诺档首轮**）+ 接管资源可用（失败能补偿）。任一不满足即不测活。
...
> ⚠️ **排队时长读策略字段，不读等级名**（A1 裁决）：实现一律取该请求别名所映射策略行的 `routing_policies.no_resource_wait_ms`。下表的"金/银/铜"是 **M0 种子模板的默认取值示例**，不是运行时判据——单级 SLA 下只有 `is_committed` 那一档真正被承诺。

| 种子模板 | `no_resource_wait_ms` 默认 | 无渠道时 |
| --- | --- | --- |
| 承诺档（`is_committed=true`，种子里对应"金"） | 15000（最多等 15s 等渠道恢复） | 返回明确"服务暂不可用" + 建议重试时间 + 事件编号 |
| 中间档（种子里对应"银"） | 5000 | 同上 |
| 无承诺档（`canary_eligible=true`，种子里对应"铜"） | 0（直接返回） | 同上 |

任何档位**不偷换低配模型、不使用不合规渠道**（FR-096 硬性）。
...
| 接管保留 | 每模型 ≥ 一个健康快渠道的**承诺档峰值并发** 10% 专用接管 | SLA 最后防线（FR-029/032）。**读 `is_committed` 不读等级名** |
...
| 情形 | 行为 |
| --- | --- |
| 两列皆空 | 过滤序 7 **直接放行**，不做任何保留判定；dispatch 走 [02 变体 B](./02-data-model.md)：`cap_res` 直通、**不碰 `resource_health`**，但 **`capacity_claims` 照写** |
| 已登记 | 按 §4.2 三档比例扣减，用与 canary 同构的 **DB 原子计数**执行；claim 同样写 |

> ⚠️ **两种情形都写 claim，区别只在要不要判闸与计数**（第 42 轮消歧：原文只说"直接放行"，
> 与 02 的变体 B 结构对不上，开发无从判断该不该写 claim）。
> claim 是**租约与悬挂回收的载体**——回收任务与崩溃恢复都靠它找孤儿 attempt。
> 未登记容量的 binding 若不写 claim，它上面的请求崩溃后**没有任何东西能发现并回收**。

- **为什么允许这种降级**：多数中转站不公开 RPM/并发上限，强行要求登记会让大部分渠道不可用。宁可"没保留"也不要"假装有保留"。
- **必须让运维看见**：`/admin/bindings` 列表与渠道详情**须显式标注"未登记容量 → 保留未生效"**（[09](./09-admin-api.md)）。否则运维会以为保留在工作，而实际**承诺档流量**随时可能被日常请求挤掉。
...
| 3 | 容量保留与订阅引流上限的叠加冲突 | ✅ **已定**：接管保留优先级最高，订阅倾斜不得侵占接管/**承诺保留**（§4.2 显式排除） |
