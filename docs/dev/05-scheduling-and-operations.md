...
| 维度 | 一期做法 | 备注 |
| --- | --- | --- |
| 渠道健康数据来源 | **仅被动累积**——只统计真实业务请求产生的 attempt 结果（成功率/TTFT/错误码），不主动发探测请求 | §3.1 样本门槛照常适用；样本不足即 `low_confidence=true` 降权 |
| 不确定渠道（新增/冷却满） | **不自动放量试探**。新渠道由运维**手工启用**（[09 `/admin/bindings`](./09-admin-api.md) 置 `enabled`），冷却到期后按 §3.2 恢复规则回到候选，靠真实流量重新积累样本 | 无 `probe_kind='probe'` 的 attempt |
| 配额/余额 unknown | 默认排除出候选（FR-118），**不靠测活消除不确定性**，靠 collector 采集刷新 | [04](./04-collector-adapter.md) |
| 记账模式 | 无测活即无"测活拖慢会话"，FR-073/074/075 的补偿语义一期无触发场景 | 二期随 §2.2 一并启用 |

> **一期取舍的代价（须知晓，非"不做也能安全跑"）**：长期无流量的渠道样本会持续陈旧，其真实可用性只能在下次被真实请求选中时才暴露；这意味着**故障发现依赖真实请求踩坑**。一期以"被动健康 + 手工启用 + 冷却/滞回"控制爆炸半径，可接受；若运营中出现"陈旧渠道频繁首次即失败"，即为二期启用 §2 的触发信号。
