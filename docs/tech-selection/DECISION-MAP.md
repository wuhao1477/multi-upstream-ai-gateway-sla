...
7. 能够识别上游订阅计划、共享额度和有效期，或提供可靠外部边界承载订阅决策。
...
- “成本控制”必须核查是否支持真实价格、倍率、账号余额、Key 额度、订阅固定费用、到期风险和失败成本，而非仅按静态 Token 单价统计。
...
已完成。矩阵按 FR-001～FR-105 分组，区分原生覆盖、L0 外置、L1/L2 和 L3 核心改造；已加入 NewAPI、Sub2API 和 FR-033～039 上游订阅调度。全部候选的价格可信版本、共享余额/订阅额度、到期预测、会话、测活和 SLA 账目仍需外部控制组件。见 [PRD 能力覆盖矩阵](./research/capability-matrix.md)。
...
已完成。同步请求前准入、逐请求资源选择、订阅加权、首字前取消/切换必须由外部 SLA 决策网关完成；价格/订阅采集、余额核对、到期预测、测活和告警走异步控制路径；AxonHub Webhook 仅作异步输入。见 [外部 SLA 控制边界](./research/control-boundary.md)。
...
已完成。唯一推荐仍为 AxonHub + 外部 SLA 决策网关，AxonHub 保持 L0。新增上游订阅需求要求外部层维护固定费用、共享额度、有效期、到期预测和引流上限；Aether、Sub2API、OmniRoute 的局部额度能力不足以改变推荐。完整结论见 [技术选型结论](./TECHNICAL-SELECTION.md)。
...
## #17: PRD v1.1 上游订阅需求影响

Blocked by: #9, #10, #11, #13

Type: Decision

### Question

新增订阅固定费用、共享额度、有效期、到期未用预测和成本限制后，现有技术选型是否仍可靠？

### Answer

已完成。Aether 具备月度额度、重置和到期字段，但当前候选过滤不排除过期额度；Sub2API 的 OpenAI 调度器有临近窗口重置优先的 `Reset` 因子；OmniRoute 有额度窗口和 `resetWindowAffinity`。这些能力均缺少适用于二十个异构上游的固定费用分摊、通用共享订阅池和预计到期未用额度决策。

因此外部订阅账本和订阅决策成为必要能力，但执行数据面的关键判据没有变化。AxonHub 单渠道 Profile 仍提供最直接的 L0 精确资源选择，最终推荐保持 `AxonHub + 外部 SLA 决策网关`。
