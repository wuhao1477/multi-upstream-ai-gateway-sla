...
`docs/dev/04` §3.1 读出了什么;真站点返回别的,全套验收(2026-09-13 实测 22+101+35 = 158 项,
另加一条日志无明文的 shell 断言;数字以脚本跑完打印的那行为准,这里只是当时的快照)
全绿也发现不了。
[`docs/acceptance/P1-evidence.md`](docs/acceptance/P1-evidence.md) §3.1 里那个
"第三族有包级测试、真实覆盖为零"的缺口就是这个坑的现成例子 —— 它一直挂在那儿,
而验收一直是绿的。**注意它后来是被"移除"结清的**(2026-08-29 该站不再纳管,
整族删掉),不是被"填上"的 —— 那族的真实形态至今没人确认过。
...
| 真库只读验收(35 项) | ❌ | GitHub runner 到不了内网 <internal-db-host> |
...
于是 `ui-stack.sh` 的 123 项(22 + 101)与真库 35 项都是**本地验收**,结论写进 PR/提交说明。
